...
5. **确认必须用 `ask_user` 工具**：禁止用文本提问代替 `ask_user` 调用——文本提问不会暂停执行，用户无法介入。调用 `ask_user` 的同一轮中禁止执行写入工具。
...
| 行为 | 说明 | 应对方式 |
|------|------|---------|
| `/` 表示层级分隔 | 标签名中的 `/` 是层级分隔符，WPS 自动将其解析为父子关系 | 写完整路径调用 `manage_note_tags({ note_id, add: ["工作/项目"] })` |
